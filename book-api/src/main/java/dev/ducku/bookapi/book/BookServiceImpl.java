...
public class BookServiceImpl implements BookService {
    ...
    public PageResponse<BookResponse> findAllBooks(int page, int size, Authentication loggedUser) {
        ...
        Page<Book> bookPage = repository.findAllDisplayableBooks(pageable, user.getId());
    ...
    public PageResponse<BookResponse> findAllBooksByOwner(int page, int size, Authentication loggedUser) {
        User user = (User) loggedUser.getPrincipal();
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<Book> bookPage = repository.findAll(withOwnerId(user.getId()), pageable);
        List<BookResponse> bookResponses = bookPage.stream()
                .map(bookMapper::toBookResponse)
                .toList();
        return PageResponse.<BookResponse>builder()
    ...
    public Integer borrowBook(Integer bookId, Authentication loggedUser) {
        ...
        final boolean isAlreadyBorrowedByLoggedUser = transactionHistoryRepository.isAlreadyBorrowedByUser(bookId, user.getId());
...
//        }
        validateIsEquals(book.getOwner().getId(), user.getId(), "You cannot borrow or return your own book");
...
//        }
        validateIsEquals(book.getCreatedBy(), user.getId(), "You cannot approve the book that you do not own");
